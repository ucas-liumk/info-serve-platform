...
| 0.3.6 | —（无 SQL） | UTM 指纹增量 / Nacos `ry-config` | 正式包仅适配已完成五服务拆分的 UTM `f632f96` 现场快照，交付前端静态资源、基线凭据、升级与回退脚本，不重复投放 Nacos。其他存量环境仍须用 `deploy/scripts/nacos-publish.sh` 按 dev/prod 双 namespace 发布 portal-kernel/appcenter/forum/requiredknowledge/resources.yml 与 ruoyi-gateway.yml（禁 SQL 直写）；可选清理 ruoyi-portal.yml。 |
...
| 0.3.8 | `0.3.8-resource-multi-category.sql` | PG `ry-cloud` | 待投放。资料多分类：新建 `info_resource_category_link` 关联表（带 tenant_id）并从存量 `category_id` 回填（幂等可重放）；`category_id` 保留为主分类 |
