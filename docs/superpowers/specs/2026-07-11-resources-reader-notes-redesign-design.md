...
- 日期：2026-07-11
- 状态：设计定稿（brainstorming 四节逐节确认 + Fable 评审修订折入）
- 范围：总体蓝图（五期）+ 一期详设（三栏阅读工作台）
...
| 三期 | 文件处理增强 | 转换管线插拔化落地：OFD 转换器（ofdrw，零新增服务）、离线 OCR 双层 PDF（PaddleOCR 自研装配 vs OCRmyPDF+中文包，实测定夺）、标签体系、全文检索（PG tsvector，含 OCR 文本） |
| 四期 | 社区 | 笔记快照发布（**按「投影可再生」原则设计**）、笔记广场、文内社区笔记叠加层、点赞/锚定评论（BC 内表，先例 `info_forum_like`）、一键导入；通知走 kernel 消息 scene（`portal.topic` 事件）；**关注/认证的归属四期前与 kernel 裁决**（跨模块社交图谱可能属公共能力） |
| 五期 | 协同 | Hocuspocus（Node）+ Yjs 实时协作；Y.Doc 升格为真相源，Tiptap JSON 降为投影 |
