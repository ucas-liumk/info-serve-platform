...
const normalizeKey = (value?: string | number) => String(value || '').toLowerCase().replace(/[^a-z0-9]/g, '');
...
const tagList = computed(() => (props.app.tags || '').split(',').map((item) => item.trim()).filter(Boolean).slice(0, 3));
...
.app-logo {
  ...
  color: #fff;
  box-shadow: 0 10px 22px rgba(42, 105, 210, 0.18), inset 0 1px 0 rgba(255, 255, 255, 0.3);
}
