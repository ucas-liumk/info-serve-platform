...
| `ruoyi-portal` | 8107 | 门户内核、应用中心、资料共享、服务论坛 |
...
架构法源是 [docs/architecture/bounded-contexts.md](./docs/architecture/bounded-contexts.md)。当前目标形态是在 `ruoyi-portal` 内按限界上下文分包，而不是把每个门户业务都拆成微服务。

| 限界上下文 | 包名 | 表前缀 | 职责 |
| --- | --- | --- | --- |
| Portal Kernel | `kernel` | `portal_` | 门户身份、消息通知、收藏、统计聚合、模块注册表 |
| AppCenter | `appcenter` | `app_` | 应用目录、离线包、需求收集 |
| Resources | `resources` | `info_` / `res_` | 资料上传、分类、审核、预览、下载 |
| Forum | `forum` | `forum_` | 版块、主题、回复、论坛通知 |
...
这些规则由 ArchUnit 测试强制，测试文件位于 `source/ruoyi-modules/ruoyi-portal/src/test/java/org/dromara/portal/arch/BcBoundaryTest.java`。
...
涉及 `ruoyi-portal` 的变更还必须显式关闭跳过测试：
...
| `ruoyi-portal` | ArchUnit：`mvn -pl ruoyi-modules/ruoyi-portal -am -DskipTests=false test` |
